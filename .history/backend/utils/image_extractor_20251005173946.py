...
class ImageExtractor:
    ...
    def extract_from_pdf(self, pdf_bytes):
        ...
        try:
            ...
            for page_num in range(len(pdf_document)):
                ...
                # 그리드로 예상 개수 확인
                expected_count = self._estimate_grid_count(image)
                print(f"  Expected products (grid estimate): {expected_count}")
                
                # 누락된 제품이 있으면 그리드 방식으로 보완
                if len(boxes) < expected_count * 0.8:  # 80% 미만이면
                    print(f"  Detection rate too low, using hybrid approach")
                    grid_boxes = self._get_grid_boxes(image)
                    # 윤곽선으로 찾은 것과 겹치지 않는 그리드 박스 추가
                    boxes = self._merge_boxes(boxes, grid_boxes)
                    print(f"  After hybrid: {len(boxes)} boxes")
    ...
    def _estimate_grid_count(self, image):
        """
        그리드 패턴으로 예상 제품 개수 추정
        """
    ...
    def _get_grid_boxes(self, image):
        """
        그리드 방식으로 박스 좌표 생성
        """
        height, width = image.shape[:2]
    ...
    def _merge_boxes(self, detected_boxes, grid_boxes):
        """
        검출된 박스와 그리드 박스를 병합
        겹치지 않는 그리드 박스만 추가
        """
        merged = list(detected_boxes)
        
        for gx, gy, gw, gh in grid_boxes:
            # 이 그리드 박스가 기존 박스와 겹치는지 확인
            overlap = False
            for dx, dy, dw, dh in detected_boxes:
                ...
                if x1 < x2 and y1 < y2:
                    ...
                    if inter_area / grid_area > 0.3:  # 30% 이상 겹침
                        overlap = True
                        break
        ...
        # 정렬
        merged.sort(key=lambda b: (b[1] // 300, b[0]))
