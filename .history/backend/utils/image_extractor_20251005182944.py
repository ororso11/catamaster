...
class ImageExtractor:
    ...
    def _extract_with_precise_grid(self, image, ocr_texts):
        ...
        # 정밀한 비율 계산 (여러 테스트 기반)
        # 상단 헤더 영역을 더 크게
        header_ratio = 0.16  # 13.5% -> 16%
        sidebar_ratio = 0.02  # 3.5% -> 2%
        ...
        print(f"  헤더: {header}px, 사이드바: {sidebar}px")
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                # OCR 텍스트 매칭 (이 셀 영역 내의 텍스트 찾기)
                cell_texts = []
                for ocr in ocr_texts:
                    if (x_start <= ocr['x'] <= x_start + cell_width and
                        y_start <= ocr['y'] <= y_start + cell_height):
                        cell_texts.append(ocr['text'])
                
                # 제품명 결정
                if cell_texts:
                    # COB, SMD 등이 포함된 텍스트 우선
                    product_name_candidates = [t for t in cell_texts 
                                              if any(k in t.upper() for k in ['COB', 'SMD', 'LED', 'MR'])]
                    if product_name_candidates:
                        product_name = product_name_candidates[0]
                    else:
                        product_name = cell_texts[0]
                else:
