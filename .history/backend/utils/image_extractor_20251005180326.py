...
class ImageExtractor:
    def __init__(self):
        self.min_image_size = 150  # 150으로 상향
        self.max_image_size = 1500  # 1500으로 하향
        self.min_area = 30000  # 최소 면적 추가
        self.target_aspect_ratio = (0.7, 1.5)  # 가로세로 비율 제한
            ...
            for page_num in range(len(pdf_document)):
                ...
                for img_index, img in enumerate(image_list):
                    try:
                        ...
                        w, h = pil_image.size
                        area = w * h
                        print(f"  Image {img_index}: {w}x{h} (area={area})")
                        ...
                        print(f"    -> ACCEPTED as product {len(products) + 1}")
    ...
    def _is_product_image(self, image):
        ...
        if aspect_ratio < self.target_aspect_ratio[0] or aspect_ratio > self.target_aspect_ratio[1]:
            return False
        ...
        # 평균 밝기 체크 (완전 흰색/검정 배경 제외)
        avg_brightness = sum(img_array) / len(img_array)
        if avg_brightness > 250 or avg_brightness < 10:
            return False
