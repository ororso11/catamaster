...
class PDFParser:
    ...
    def extract_products_from_page(self, page, page_num):
        ...
        # 왼쪽 사이드바 제외 (약 6%)
        sidebar_width = int(img.width * 0.06)
        working_width = img.width - sidebar_width
        
        # 상단 헤더 제외 (약 8%)
        header_height = int(img.height * 0.08)
        ...
        for row in range(ROWS):
            for col in range(COLS):
                ...
                # 메인 제품 이미지 영역: 셀 내부 15%~60% 구간
                # (상단 Ø 표시 제외, 하단 작은 아이콘/텍스트 제외)
                img_y_start = y_start + int(cell_height * 0.15)
                img_y_end = y_start + int(cell_height * 0.60)
                
                # 좌우 여백 최소화 (2%만)
                padding_x = int(cell_width * 0.02)
                ...
                try:
                    ...
                    if avg_brightness > 240 or avg_brightness < 15:
                        logger.info(f"   셀 [{row},{col}]: 빈 영역 스킵")
                        continue
                    
                    # 고해상도 유지 (리사이즈 안 함)
                    # Base64 인코딩
                    buffered = BytesIO()
                    product_img.save(buffered, format="JPEG", quality=95)
                    img_str = base64.b64encode(buffered.getvalue()).decode()
                    img_data_url = f"data:image/jpeg;base64,{img_str}"
                    
                    # 텍스트 영역: 셀의 75%~100% 구간
                    text_y_start = y_start + int(cell_height * 0.75)
                    text_rect = fitz.Rect(
                        x_start / zoom,
                        text_y_start / zoom,
