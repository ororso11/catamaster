...
class ImageExtractor:
    ...
    def _extract_text_with_vision(self, img_bytes):
        ...
        try:
            ...
            # Y좌표로 정렬
            text_blocks.sort(key=lambda t: t['center_y'])
            
            # 제품별로 그룹화 (Y좌표 기준 - 같은 행)
            product_groups = []
            current_group = []
            last_y = -1
            ...
            for block in text_blocks:
                # 새로운 행 시작
                if last_y > 0 and abs(block['center_y'] - last_y) > y_threshold:
                    if current_group:
                        # COB, SMD 등이 포함된 그룹만 저장
                        group_text = ' '.join([t['text'] for t in current_group])
                        if any(k in group_text.upper() for k in ['COB', 'SMD', 'LED', 'MR']):
                            product_groups.append(group_text)
                        current_group = []
