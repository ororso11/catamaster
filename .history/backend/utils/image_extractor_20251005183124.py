...
class ImageExtractor:
    ...
    def _extract_with_precise_grid(self, image, ocr_texts):
        ...
        # 다양한 비율 테스트
        test_configs = [
            {'header': 0.14, 'sidebar': 0.02, 'name': 'Config 1'},
            {'header': 0.16, 'sidebar': 0.02, 'name': 'Config 2'},
            {'header': 0.18, 'sidebar': 0.02, 'name': 'Config 3'},
            {'header': 0.15, 'sidebar': 0.01, 'name': 'Config 4'},
            {'header': 0.17, 'sidebar': 0.03, 'name': 'Config 5'},
        ]
        ...
        for config in test_configs:
            header = int(height * config['header'])
            sidebar = int(width * config['sidebar'])
            ...
            print(f"\n  {config['name']}: 헤더={header}, 사이드바={sidebar}, 셀={cell_width}x{cell_height}")
            ...
            for row in range(rows + 1):
                ...
                cv2.line(debug_image, (sidebar, y), (width, y), (0, 255, 0), 2)
            
            for col in range(cols + 1):
                x = sidebar + (col * cell_width)
                cv2.line(debug_image, (x, header), (x, height), (0, 255, 0), 2)
            
            # 디버그 이미지 저장
            debug_path = f'debug_grid_{config["name"].replace(" ", "_")}.jpg'
            ...
            all_results.append({'config': config, 'products': products, 'count': len(products)})
        
        # 가장 많은 제품을 추출한 설정 사용
        best = max(all_results, key=lambda r: r['count'])
        print(f"\n  최선의 설정: {best['config']['name']} ({best['count']}개 제품)")
        print(f"  → debug_grid_{best['config']['name'].replace(' ', '_')}.jpg 파일을 확인하세요")
