...
async function processPDF(file) {
    ...
    reader.onload = async function(e) {
        ...
        const extractedProducts = await extractProductsFromPDF(pdfData);
        
        // 2단계: Firebase 구조 생성
        showProcessingStatus('데이터베이스 생성 중...');
...
// PDF에서 제품 데이터 추출 (시뮬레이션)
async function extractProductsFromPDF(pdfData) {
...
function downloadFiles(catalogId) {
    ...
    Object.keys(catalog.files).forEach(filename => {
        setTimeout(() => downloadFile(catalogId, filename), 100);
    });
}
...
function getStyleCSS() {
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    header { color: white; padding: 15px 30px; display: flex; justify-content: space-between; align-items: center; }
    .logo { font-size: 18px; letter-spacing: 6px; }
    .product-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 20px; padding: 20px; }
    .product-card { background: white; border: 1px solid #e0e0e0; padding: 20px; cursor: pointer; }
    footer { background: #2a2a2a; color: #999; padding: 40px; text-align: center; }`;
}
