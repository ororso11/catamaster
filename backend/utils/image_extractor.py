...
class ImageExtractor:
    ...
    def _extract_with_position_matching(self, page, pdf_document, text_blocks, page_width, page_height):
        ...
        for img_pos in image_positions:
            try:
                ...
                print(f"    사용된 텍스트 블록: {len(product_info['used_indices'])}개")
    ...
    def _find_text_below_image(self, img_pos, text_blocks, used_text_blocks):
        ...
        for line in lines[2:]:
            ...
            if re.search(r'\d+', clean_line) or any(unit in clean_line for unit in ['W', 'mm', 'V', '°', 'K', 'lm', 'COB', 'SMD', 'IP']):
